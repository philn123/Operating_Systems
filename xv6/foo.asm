...
main(int argc, char *argv[])
{
  ...
  if(argc < 2 )
  1a:	0f 8e 8a 00 00 00    	jle    aa <main+0xaa>
    n = 1;       //default value
  else
    n = atoi ( argv[1] ); //from command line
  20:	83 ec 0c             	sub    $0xc,%esp
  23:	ff 70 04             	pushl  0x4(%eax)
  26:	e8 95 02 00 00       	call   2c0 <atoi>
  if ( n < 0 || n > 20 )
  2b:	83 c4 10             	add    $0x10,%esp
  2e:	83 f8 14             	cmp    $0x14,%eax
  double x = 0,  z;
  ...
  if ( n < 0 || n > 20 )
  33:	0f 86 a9 00 00 00    	jbe    e2 <main+0xe2>
    n = 2;
  39:	bf 02 00 00 00       	mov    $0x2,%edi
  3e:	31 db                	xor    %ebx,%ebx
  40:	eb 31                	jmp    73 <main+0x73>
  ...
  48:	74 67                	je     b1 <main+0xb1>
      printf(1, "Parent %d creating child  %d\n", getpid(), id );
  4a:	e8 63 03 00 00       	call   3b2 <getpid>
  4f:	56                   	push   %esi
  50:	50                   	push   %eax
    n = atoi ( argv[1] ); //from command line
  if ( n < 0 || n > 20 )
  ...
  51:	83 c3 01             	add    $0x1,%ebx
    ...
    if ( id < 0 ) {
      ...
      printf(1, "Parent %d creating child  %d\n", getpid(), id );
  54:	68 c4 07 00 00       	push   $0x7c4
  59:	6a 01                	push   $0x1
  5b:	e8 30 04 00 00       	call   490 <printf>
      wait (0);
  60:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
  67:	e8 ce 02 00 00       	call   33a <wait>
  ...
  id = 0;
  for ( k = 0; k < n; k++ ) {
  6f:	39 fb                	cmp    %edi,%ebx
  71:	7d 2d                	jge    a0 <main+0xa0>
    id = fork ();
  73:	e8 b2 02 00 00       	call   32a <fork>
    if ( id < 0 ) {
  ...
  7a:	89 c6                	mov    %eax,%esi
    if ( id < 0 ) {
  7c:	79 ca                	jns    48 <main+0x48>
      printf(1, "%d failed in fork!\n", getpid() );
  7e:	e8 2f 03 00 00       	call   3b2 <getpid>
  ...
  a3:	6a 01                	push   $0x1
  a5:	e8 88 02 00 00       	call   332 <exit>
{
  int  k, n, id;
  double x = 0,  z;
  ...
  b1:	e8 fc 02 00 00       	call   3b2 <getpid>
  b6:	52                   	push   %edx
  b7:	50                   	push   %eax
  b8:	68 e2 07 00 00       	push   $0x7e2
  bd:	6a 01                	push   $0x1
  bf:	e8 cc 03 00 00       	call   490 <printf>
      for ( z = 0; z < 8000000.0; z += 0.01 )
  c4:	d9 ee                	fldz   
      printf(1, "%d failed in fork!\n", getpid() );
    } else if ( id > 0 ) {  //parent
  ...
  c6:	83 c4 10             	add    $0x10,%esp
      for ( z = 0; z < 8000000.0; z += 0.01 )
  c9:	dd 05 f8 07 00 00    	fldl   0x7f8
  cf:	90                   	nop
  d0:	dc c1                	fadd   %st,%st(1)
  d2:	d9 05 00 08 00 00    	flds   0x800
  d8:	df ea                	fucomip %st(2),%st
  da:	77 f4                	ja     d0 <main+0xd0>
  dc:	dd d8                	fstp   %st(0)
  de:	dd d8                	fstp   %st(0)
  e0:	eb be                	jmp    a0 <main+0xa0>
    n = atoi ( argv[1] ); //from command line
  if ( n < 0 || n > 20 )
    n = 2;
  x = 0;
  ...
  e4:	74 ba                	je     a0 <main+0xa0>
  e6:	e9 53 ff ff ff       	jmp    3e <main+0x3e>
  eb:	66 90                	xchg   %ax,%ax
  ed:	66 90                	xchg   %ax,%ax
  ef:	90                   	nop
